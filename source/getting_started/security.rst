...
Similar to medieval fortifications it is recommended to provide your servers with multiple layers of defenses. Be sure to use Firewalls, Strong passwords, SSH, and make sure your servers are kept up to date for all software being used. This includes the operating system, FreeSWITCH and cclpbx.


cclpbx
^^^^^^^^
The latest Debian install script configures IPTables firewall for you. cclpbx extensions set strong passwords for you by default. You can increase the password complexity using settings in Advanced -> `Default Settings`_ to increase the length of the passwords that are generated by default.
...
Security problems are fixed as they are discovered and are updated for master and the latest release. Upgrades are considered an important part of keeping the server secure. `Upgrades`_ always need to be done on the operating system, FreeSWITCH and cclpbx.
...
New install mod_xml_rpc is not enabled by default. It is recommended to run a firewall on all cclpbx servers. The latest Debian install script configures the firewall by default. However it is recommended to check to make sure it is installed and running.
...
.. _paid support: http://www.cclpbx.com
...
.. _Setup Let's Encrypt with cclpbx: /en/latest/getting_started/lets_encrypt.html
